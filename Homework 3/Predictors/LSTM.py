...
def fetch_stock_data(stock_code):
    print(stock_code)
    cursor = collection.find({'company_name': stock_code}, {'_id': 0, 'date': 1, 'last_trade_price': 1, 'min_price': 1, 'max_price': 1, 'volume': 1})
...
def train_and_save_lstm_model(stock_code):
    ...
    update_last_updated(stock_code)

def update_last_updated(stock_code):
...
def generate_prediction_plot(y_test, predicted_price, stock_code):
...
def predict_stock_price(stock_code):
    try:
        ...
        if not os.path.exists(model_path):
            train_and_save_lstm_model(stock_code)
        ...
        generate_prediction_plot(y_test, predicted_price, stock_code)
        current_price = stock_data["last_trade_price"].iloc[-1]
        signal = generate_signal(current_price, predicted_price[-1][0])
